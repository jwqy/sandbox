...
04) Set the width of the same elements to 80 pixels.

05) After changing the position of the <header> element to fixed, the contents of the entire site after it shifted upwards.
	Set the position of <main> so that we can position it relatively.

06) The <header> has disappeared behind the <main>.
	Use z-index to make the <header> visible.

07) Now the navigation bar looks good, but it is blocking the title at the top of the page.
	Offset <main> by 80 pixels from the top.

08) Now, fix up the supporting element style below the image.
...
09) Inspect the .supporting .col elements—they don’t seem to be flowing horizontally yet because they have no set width. 
	Set the width and height of .supporting .col elements to 200 pixels.
