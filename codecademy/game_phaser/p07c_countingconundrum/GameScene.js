class GameScene extends Phaser.Scene {
	...
	create() {
		...
		for (let i = 1; i < 10; i++) {
				...
				onComplete: function () {
					...
					currentCircle.on('pointerup', () => {
						if (gameState.counter === currentCircle.number) {
							++gameState.counter;
							++gameState.correct;
							//extra: use tween instead of destroy directly
							// currentCircle.destroy();
							currentCircle.rightTween.restart();
						} else {
							++gameState.incorrect;
			...
			//extra: fade correct circle
			currentCircle.rightTween = this.tweens.add({
				targets: currentCircle,
				paused: true,
				scaleX: 0,
				scaleY: 0,
